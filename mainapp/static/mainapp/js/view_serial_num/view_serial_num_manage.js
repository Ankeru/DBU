...
his_radio_element.change(
        function change_data(){
            ...
            s_table_first_row_td_list.eq(1).find("input[type='date']").eq(0).val(
                o_table.eq(number_in_o_table).find('td').eq(1).find('input').val());       
            s_table_first_row_td_list.eq(2).find("input[type='date']").eq(0).val(
                o_table.eq(number_in_o_table).find('td').eq(2).find('input').val());  
            for (let i = 4; i < 7; i++) { 
                    s_table_first_row_td_list.eq(i).find('select').val(
                        o_table.eq(number_in_o_table).find('td').eq(i).text());     
                }    
            s_table_first_row_td_list.eq(3).find('textarea').val(
                o_table.eq(number_in_o_table).find('td').eq(3).text());              
            s_table_first_row_td_list.eq(7).find('textarea').val(
...
// }    
// const his_button_submit_element = $('#submit_history_change');
// his_button_submit_element.click(hide_block_and_copy_data);
