		...
		<?php
			...
			if ($db=mysqli_connect("localhost","mysql_boards","75020141551q1619w","boardsDataBase")) #подключение к бд: хост,логин,пароль, название бд
				...
				if (!mysqli_set_charset($db,'utf8'))
			        ...
			        if (!($IdCookie===""))//Для кнопки "Изменить"
                                	if(!($SerialNumCookie===""))
						...
						echo "</script>";
                                ...
                                if (!($DeletionIdCookie===""))//Для кнопки "Удалить"
                                        if($PasswordCookie===$AdminPassword)
                                                {
						$sql = 'DELETE FROM `objects` WHERE `objId`='.$DeletionIdCookie;
						echo "<script type='text/javascript'>\n";
                                                echo "window.alert(".$sql.")";
                                                echo "</script>";
                                        	 mysqli_query($db,$sql);
                                                echo "<script type='text/javascript'>\n";
                                                setrawcookie("id","");
...
/*Изменяемая часть */
					...
					echo '<input id="radChoice" type="radio" name="typeRadio" onclick="f_radio(`'.$PasswordCookie.'`,`choice`);" checked> Выбор
	                               ...
	                               /* <input id="radDel" type="radio" name="typeRadio" onclick="f_radio(`'.$PasswordCookie.'`,`delete`);"> Удаление';*/
					};
					break;
                                        case 'edit': case '':{
					 echo 'Тип  <select id="objType"  name="objType" onchange="f_update('.$PasswordCookie.');" >';
                                       ...
                                       echo '<input id="radChoice" type="radio" name="typeRadio" onclick="f_radio(`'.$PasswordCookie.'`,`choice`);"> Выбор
                                        <input id="radEdit" type="radio" name="typeRadio" onclick="f_radio(`'.$PasswordCookie.'`,`edit`);" checked> Редактирование';
					...
					/*
                                         case 'delete':
					{if($PasswordCookie===$AdminPassword){
					echo '<input id="serial'.$pole[0].'" type="text" name="serial'.$pole[0].'" value="'.$pole[2].'">';
					}else echo 'Пароль введен неверно';
					
					echo '<input id="radChoice" type="radio" name="typeRadio" onclick="f_radio(`'.$PasswordCookie.'`,`choice`);"> Выбор
                                        <input id="radEdit" type="radio" name="typeRadio" onclick="f_radio(`'.$PasswordCookie.'`,`edit`);"> Редактирование
                                        <input id="radDel" type="radio" name="typeRadio" onclick="f_radio(`'.$PasswordCookie.'`,`delete`);" checked> Удаление';
	...
	<table border="1" width="1000">
		...
		<tbody>
			...
			if ($db=mysqli_connect("localhost","mysql_boards","75020141551q1619w","boardsDataBase"))#подключение к бд: хост,логин,пароль, название бд
				...
				if (!mysqli_set_charset($db,'utf8'))
			       		echo 'Charset setting fail';  
				$sql = 'SELECT `objId`,`isFree`,`serialNumber` FROM `objects` WHERE `type`="'.$cookieType .'" ORDER BY `objects`.`serialNumber` ASC';
				$rs = mysqli_query($db,$sql);
				while ($pole = mysqli_fetch_array($rs)){
					echo '<tr>';
                                        echo '<td><input id="serial'.$pole[0].'" type="text" name="serial'.$pole[0].'" value="'.$pole[2].'">
						<input type="button" name="'.$pole[0].'ChangeButton" value="Изменить" onclick="f_changeSerialNumber(`'.$pole[0].'`);"></td>';
				       
                                        if  ($pole[1]==="1")
						echo '<td>'."Доступен".'</td>';
					else {	$sql = 'SELECT `location` FROM `records`  WHERE `objId`="'.$pole[0].'" ORDER BY `records`.`recordId` DESC';
					...
					echo '<input type="button" name="'.$pole[0].'DeleteButton" value="Удалить" onclick="f_delete(`'.$pole[0].'`);">'.'</td>'; 
					echo '</tr>';
				}
				mysqli_free_result($rs); 
				mysqli_close($db); 
